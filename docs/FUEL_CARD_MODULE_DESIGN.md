...
# CURRENT IMPLEMENTATION PHASE / PRECEDENCE
...
Each structure has a first-class home in native storage (generalized model). **Implementation 1** stores the approved BVD transaction/header fields as explicit TEXT columns on `fuel_bvd` with side-by-side review; it does not require block/record JSONB tables for the current milestone.
...
# 6. PostgreSQL / JSONB native-evidence storage model (future generalization)

Postgres + JSONB is the planned storage foundation for the **generalized** provider-native layer. **BVD Implementation 1** uses explicit TEXT columns on `fuel_bvd` per `docs/FUEL_BVD_IMPLEMENTATION_1.md` and does not require this table stack yet.
...
BVD Implementation 1 (`fuel_bvd` fidelity) is the current execution focus. Only after that milestone (and live acceptance) is stable does Nationwide validate framework reuse. Only after both provider source-fidelity paths are stable do semantic/canonical math and financial posting become the next implementation focus.
...
7. Native raw values stay exact strings (ordered JSONB field arrays in the generalized model; explicit TEXT columns on `fuel_bvd` for BVD Implementation 1).
