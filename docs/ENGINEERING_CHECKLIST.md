...
- [ ] **Frontend live reload:** Run `./scripts/dev-web.sh` (or `cd apps/web && npm run dev`), open http://localhost:5173 — changes reflect without rebuilding; API must be on port 8000 (e.g. dev stack with docker-compose.dev.yml).
