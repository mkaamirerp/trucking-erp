...
- **`DeprecatedDispatchPage.tsx`** — Deprecated legacy board: ribbon tabs, **table** or **board** view, driver column, and load cards/columns. Uses **`getDispatchBoard`**, **`listTrucks`**, **`listDrivers`**, and **`listTrailers`** for compatibility display. **Unassigned** loads navigate to **`LoadWorkspacePage`** at `/loads/:id` without an assignment query mode. Operational assignment continues through **Create Planned Trip** and **TripWorkspacePage**; do not restore Load-status assignment. Other statuses open an in-page summary modal with **Edit load**. **New load** → `/loads/new`.
