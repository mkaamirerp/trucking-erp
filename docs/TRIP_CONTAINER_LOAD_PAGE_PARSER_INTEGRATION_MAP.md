...
| **Dispatch board** | `DeprecatedDispatchPage.tsx` ~620–726; comment ~3 | Unassigned: ` /loads/{id}?dispatchAssign=1` | `LoadWorkspacePage` + `DispatchAssignmentStrip` | **Edit** (assign resources) | PDF possible on same page | **Dispatch** — assignment strip; trip exists after status moves into dispatched path (backend) |
