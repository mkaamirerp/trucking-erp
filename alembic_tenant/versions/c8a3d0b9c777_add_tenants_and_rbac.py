...
def upgrade() -> None:
    ...
    indexes = {idx["name"] for idx in inspector.get_indexes("permissions", schema="public")}
    ...
    if "roles" not in tables:
        ...
        existing_columns = {col["name"] for col in inspector.get_columns("roles", schema="public")}
        if "scope" not in existing_columns:
            op.add_column("roles", sa.Column("scope", sa.String(length=20), nullable=False, server_default=sa.text("'tenant'")), schema="public")
        if "tenant_id" not in existing_columns:
    ...
    if "role_permissions" not in tables:
        ...
        tables.add("role_permissions")
    inspector = sa.inspect(op.get_bind())
    indexes = {idx["name"] for idx in inspector.get_indexes("role_permissions", schema="public")}
    existing_columns = {col["name"] for col in inspector.get_columns("role_permissions", schema="public")}
    if "ix_role_permissions_role_id" not in indexes and "role_id" in existing_columns:
    ...
    inspector = sa.inspect(op.get_bind())
    existing_columns = {col["name"] for col in inspector.get_columns("audit_log", schema="public")}
    indexes = {idx["name"] for idx in inspector.get_indexes("audit_log", schema="public")}
    if "ix_audit_log_tenant_id" not in indexes and "tenant_id" in existing_columns:
        op.create_index("ix_audit_log_tenant_id", "audit_log", ["tenant_id"], unique=False, schema="public")
    if "ix_audit_log_actor_user_id" not in indexes and "actor_user_id" in existing_columns:
    ...
    inspector = sa.inspect(op.get_bind())
    doc_file_columns = {col["name"] for col in inspector.get_columns("driver_document_files", schema="public")}
    if "deactivated_at" not in doc_file_columns:
        op.add_column(
            "driver_document_files",
            sa.Column("deactivated_at", sa.DateTime(timezone=True), nullable=True),
            schema="public",
        )
    if "deactivated_reason" not in doc_file_columns:
        op.add_column(
            "driver_document_files",
