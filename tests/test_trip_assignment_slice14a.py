"""Decision 14A: PUT /trips/{id}/assignment — integration tests."""
...
from app.constants.trip_dispatch import TRIP_CONTAINER_STATUS_ASSIGNED, TRIP_CONTAINER_STATUS_PLANNED
...
@pytest.mark.asyncio
class TestTripAssignmentSlice14aHttp:
    async def test_put_assignment_happy_path(self, client: AsyncClient, locked_prefix: str, override_auth_tenant):
        ...
        assert body["status"] == TRIP_CONTAINER_STATUS_ASSIGNED
        assert body["assigned_at"] is not None

    async def test_put_assignment_invalid_trip(self, client: AsyncClient, override_auth_tenant):
        r = await client.put(
            "/api/v1/trips/999999991/assignment",
            headers=AUTH_HEADERS,
            json={"driver_id": None, "truck_id": None, "trailer_id": None},
        )
        assert r.status_code == 404

    async def test_put_assignment_cancelled_trip_rejected(
    ...
    async def test_put_assignment_invalid_driver_rejected(self, client: AsyncClient, override_auth_tenant):
        """Non-existent / wrong-tenant driver id fails validation (404 Driver not found)."""
        assets = await _first_driver_truck_trailer(client)
        if not assets:
            pytest.skip("no driver/truck/trailer")
        _, t_id, r_id = assets
        ...
        r_put = await client.put(
            f"/api/v1/trips/{trip_id}/assignment",
            headers=AUTH_HEADERS,
            json={"driver_id": 999999992, "truck_id": t_id, "trailer_id": r_id},
        )
        assert r_put.status_code == 404

    async def test_put_assignment_no_dispatch_trips_no_load_dispatched(
        self, client: AsyncClient, locked_prefix: str, override_auth_tenant
    ):
        ...
        lr = await client.get(f"/api/v1/loads/{load_id}", headers=AUTH_HEADERS)
        assert lr.status_code == 200
        assert lr.json().get("status") == load_status_before

    async def test_put_assignment_writes_audit_event(self, client: AsyncClient, locked_prefix: str, override_auth_tenant):
        url = _tenant_async_url()
        if not url:
            pytest.skip("no tenant db url")
        assets = await _first_driver_truck_trailer(client)
                    ...
                    audit_before = (
                        await session.execute(
                            text(
                                ...
                                "AND entity_type = 'trip' AND action = 'trip_assignment_updated'"
                            ),
                            {"tid": tenant_id},
                        )
                    ).scalar() or 0
