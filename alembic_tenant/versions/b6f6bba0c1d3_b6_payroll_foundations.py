...
def upgrade() -> None:
    ...
    if "pay_periods" not in tables:
        ...
        tables.add("pay_periods")
    inspector = sa.inspect(bind)
    pay_period_indexes = {idx["name"] for idx in inspector.get_indexes("pay_periods")} if "pay_periods" in tables else set()
    if "ix_pay_periods_tenant_id" not in pay_period_indexes and "pay_periods" in tables:
        op.create_index("ix_pay_periods_tenant_id", "pay_periods", ["tenant_id"])
    if "ix_pay_periods_status" not in pay_period_indexes and "pay_periods" in tables:
        op.create_index("ix_pay_periods_status", "pay_periods", ["status"])
    if "ix_pay_periods_dates" not in pay_period_indexes and "pay_periods" in tables:
        op.create_index("ix_pay_periods_dates", "pay_periods", ["tenant_id", "start_date", "end_date"])

    if "pay_profiles" not in tables:
        op.create_table(
            "pay_profiles",
            sa.Column("id", sa.Integer(), primary_key=True, nullable=False),
            sa.Column(
        ...
        tables.add("pay_profiles")
    inspector = sa.inspect(bind)
    pay_profile_indexes = {idx["name"] for idx in inspector.get_indexes("pay_profiles")} if "pay_profiles" in tables else set()
    if "ix_pay_profiles_tenant_driver" not in pay_profile_indexes and "pay_profiles" in tables:
        op.create_index("ix_pay_profiles_tenant_driver", "pay_profiles", ["tenant_id", "driver_id"])
    if "ix_pay_profiles_active" not in pay_profile_indexes and "pay_profiles" in tables:
        op.create_index("ix_pay_profiles_active", "pay_profiles", ["tenant_id", "is_active"])

    if "pay_entries" not in tables:
        op.create_table(
            "pay_entries",
            sa.Column("id", sa.Integer(), primary_key=True, nullable=False),
            sa.Column(
        ...
        tables.add("pay_entries")
    inspector = sa.inspect(bind)
    pay_entry_indexes = {idx["name"] for idx in inspector.get_indexes("pay_entries")} if "pay_entries" in tables else set()
    if "ix_pay_entries_tenant_period" not in pay_entry_indexes and "pay_entries" in tables:
        op.create_index("ix_pay_entries_tenant_period", "pay_entries", ["tenant_id", "pay_period_id"])
    if "ix_pay_entries_tenant_driver" not in pay_entry_indexes and "pay_entries" in tables:
        op.create_index("ix_pay_entries_tenant_driver", "pay_entries", ["tenant_id", "driver_id"])
    if "ix_pay_entries_tenant_entry_type" not in pay_entry_indexes and "pay_entries" in tables:
        op.create_index("ix_pay_entries_tenant_entry_type", "pay_entries", ["tenant_id", "entry_type"])
    if "ix_pay_entries_active" not in pay_entry_indexes and "pay_entries" in tables:
