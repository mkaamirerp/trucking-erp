...
- **Custody foundation (Slice 1 shipped):** tenant **`terminals`** table; append-only **`load_custody_events`**; Load snapshot columns (`custody_owner`, `custody_trip_id`, `custody_terminal_id`, `custody_placement`, `custody_trailer_id`, `custody_since_at`, `last_custody_event_id`); read APIs `GET /loads/{id}/custody`, `GET /loads/{id}/custody-events`, `GET /terminals`; bootstrap script `python -m app.scripts.bootstrap_load_custody` (anomaly STOP, no invented yard history). **Not yet:** accept-custody / yard-handoff / take-custody mutations; bare `/activate` and `/complete` still public.
