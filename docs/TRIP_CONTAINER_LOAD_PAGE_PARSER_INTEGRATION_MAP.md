...
| **Edit / view existing load** | `App.tsx`; `LoadWorkspacePage` | `/loads/:id` | `LoadWorkspacePage` | **Edit** commercial/readiness fields (PATCH) + view notes, audit, settlement per mode | Yes — `canWorkspaceParsePdf` in manual, intake, detail | **Read-only** trip context; operational commitment continues through **Create Planned Trip** and Trip workspace. No dispatch query mode or assignment strip. |
...
| **Dispatch board** | `DeprecatedDispatchPage.tsx` | Unassigned: `/loads/{id}` | `LoadWorkspacePage` | **Edit** commercial/readiness fields; no Load-status assignment writer | PDF possible on same page | Legacy board opens the Load; operational dispatch proceeds through planned Trip / Trip workspace. |
| **“New load” from dispatch empty column** | `DeprecatedDispatchPage.tsx` ~694, ~725–726 | `navigate(…OPS.LOAD_NEW)` | `LoadWorkspacePage` | Create | Optional | None |
