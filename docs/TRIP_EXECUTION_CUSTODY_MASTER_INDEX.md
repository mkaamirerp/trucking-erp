...
- **Many-to-many continuity (LOCKED):** one Trip may carry multiple Loads; one Load may move through multiple Trips; TripLoad is the membership bridge; do not duplicate commercial Loads for multi-Trip movement. Reference: inbound multi-load → yard stage → split outbound Trips — [`trip-foundation.md`](./trip-foundation.md) §1A many-to-many scenario.
...
- **TripLoad open ≠ active (LOCKED):** **Open membership** = `status_within_trip IN ('planned','active')` **AND** `completed_at IS NULL` **AND** `removed_at IS NULL`. **Active membership** = `status_within_trip = 'active'` **AND** open. Completed keeps `removed_at` NULL and sets `completed_at`; removed sets `removed_at` only. Open alone is **not** custody / current movement / current Trip. V1 per Load: **max one** open **active**, **max one** open **planned**; unlimited completed/removed history; **active A + planned B** is **valid**; two open actives or two open planneds are **invalid**. Completing A does **not** auto-activate B. Detail: [`trip-foundation.md`](./trip-foundation.md) §1A TripLoad membership semantics. *(Supersedes older “open/active = removed_at IS NULL” wording.)*
- **`loads.active_trip_id`** is a **backend compatibility mirror** of the Load’s current **ACTIVE** Trip only — **not** planned, **not** an arbitrary open TripLoad; **UI must not “repair” drift** (membership APIs own consistency). TripLoad remains source of truth.
...
- **Exact trip completion gating (V1 header close shipped):** `POST /api/v1/trips/{id}/complete` — **`in_progress` → `completed`** only with **zero OPEN** TripLoads; sets **`trips.completed_at`**; Trip-row `FOR UPDATE` vs membership insert; terminal mutators reject `TRIP_ALREADY_COMPLETED`. **Required custody event sequences** and **RBAC** for closing trips with undelivered freight (**Decision 12**) remain future — V1 blocks while any OPEN ACTIVE membership remains (no custody escape hatch yet).
- **Custody foundation (Slice 1 shipped):** tenant **`terminals`** table; append-only **`load_custody_events`**; Load snapshot columns (`custody_owner`, `custody_trip_id`, `custody_terminal_id`, `custody_placement`, `custody_trailer_id`, `custody_since_at`, `last_custody_event_id`); read APIs `GET /loads/{id}/custody`, `GET /loads/{id}/custody-events`, `GET /terminals`; bootstrap script `python -m app.scripts.bootstrap_load_custody` (anomaly STOP, no invented yard history).
- **Custody operational transitions (Slice 2):** `POST /trips/{id}/loads/{id}/accept-custody` (unknown→trip), `.../yard-handoff` (trip→terminal + active→completed), `.../take-custody` (terminal→outbound trip). Bare public `/activate` and `/complete` return **409 `MEMBERSHIP_TRANSITION_REQUIRES_CUSTODY`**. Does not auto-start Trip execution or mutate `Load.status`.
